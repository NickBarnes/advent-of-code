...
class Valve:
    ...
    def dist(self):
        self.distances = graph.shortest_tree(self, lambda v: ((t,1) for t in v.neighbours))
...
def go(input):
    lines = [valve_re.match(line).groups() for line in parse.lines(input)]
    ...
    print(f"part 1 (total pressure released in 30 minutes): {score}")
    ...
    print(f"part 2 (total pressure released in 26 minutes with the help of Tramples, Crusher of Enemies): {score}")
