...
class Game:
    def __init__(self, s):
        ...
        for r in desc.split(';'):
            ...
            self.rounds.append(d)
    
    def possible(self, cols):
        return all(r.get(k,0) <= n for r in self.rounds for k,n in cols.items())

    def power(self):
        counts = [max(r.get(col,0) for r in self.rounds) for col in ['red','green','blue']]
...
def go(filename):
    ...
    print(sum([g.id for g in games if g.possible({"red":12, "green":13, "blue":14})]))
