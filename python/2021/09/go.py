def go(input):
    depths = [[int(c) for c in l] for l in parse.lines(input)]
    rows = len(depths)
    cols = len(depths[0])
    ...
    print(f"part 1 (risk total): {risk_total}")
    ...
    product=misc.prod(l for l,_ in basin_lens[:3])
    print(f"part 2 (product of largest three basin sizes): {product}")
